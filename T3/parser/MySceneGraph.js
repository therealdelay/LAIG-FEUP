...
MySceneGraph.prototype.onXMLReady = function()
{
  ...
  var rootElement = this.reader.xmlDoc.documentElement; // blocK <SCENE> of .xml
    ...
    if ((nodeName = children[i].nodeName) == "ROOT") {
      ...
      for (var j = 0; j < descendants.length; j++) {
        if (descendants[j].nodeName == "NODEREF")
        {
          ...
          {
            var type=this.reader.getItem(descendants[j], 'type', ['rectangle', 'cylinder', 'sphere', 'triangle', 'circle', 'patch', 'peca', 'henge']);
